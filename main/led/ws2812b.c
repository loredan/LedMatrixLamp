...
typedef struct
{
    ws2812b_strip_controller_t parent;
...
void ws2812b_set_pixel(ws2812b_strip_controller_t *strip, uint32_t index, uint8_t red, uint8_t green, uint8_t blue)
{
    ws2812b_strip_t *ws2812b = __containerof(strip, ws2812b_strip_t, parent);
...
void ws2812b_refresh(ws2812b_strip_controller_t *strip)
{
    ws2812b_strip_t *ws2812b = __containerof(strip, ws2812b_strip_t, parent);
    rmt_write_sample(ws2812b->config.channel, ws2812b->buffer, ws2812b->config.num_leds * 3, true);
}

void ws2812b_clear(ws2812b_strip_controller_t *strip)
{
    ws2812b_strip_t *ws2812b = __containerof(strip, ws2812b_strip_t, parent);
    memset(ws2812b->buffer, 0, ws2812b->config.num_leds * 3);
    return ws2812b_refresh(strip);
}

void ws2812b_del(ws2812b_strip_controller_t *strip)
{
    ws2812b_strip_t *ws2812b = __containerof(strip, ws2812b_strip_t, parent);
    free(ws2812b);
}
...
{
    uint32_t ws2812b_size = sizeof(ws2812b_strip_t) + config.num_leds * 3;
    ws2812b_strip_t *ws2812b = calloc(1, ws2812b_size);
